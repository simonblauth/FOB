...
class AdamPlus(Optimizer):
    """
    Initializes the AdamPlusELR optimizer.
    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            group.setdefault("lr_update", True)
    ...
    def _init_group(
        ...
        for p, lr_idx in zip(group["params"], group["lr_index"]):
            ...
            if len(state) == 0:
                ...
                state["elr_exp_avg"] = torch.tensor([0.0], dtype=torch.float, device=p.device)
            ...
            lrs.append(state["lr"])
            elr_exp_avgs.append(state["elr_exp_avg"])
